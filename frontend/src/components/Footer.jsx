...
export function Footer() {
  ...
  useEffect(() => {
    ...
    gsap.to(titleRef.current, {
      ...
      scrollTrigger: {
        ...
        scrub: 1,
        invalidateOnRefresh: true,
        // Prevent extra space by using these markers:
  ...
  return (
    <footer
      ref={footerRef}
      className="relative w-full min-h-[60vh] sm:min-h-[80vh] bg-white text-black px-6 sm:px-12 py-20 flex flex-col justify-between overflow-hidden" // Added overflow-hidden
    >
      {/* Navigation Section - unchanged */}
      <div className="flex flex-col sm:flex-row gap-10 sm:gap-20 text-sm sm:text-base">
        <div className="flex flex-col gap-2">
          <h3 className="mb-2 uppercase text-[#00000080] text-xs sm:text-sm">About</h3>
        ...
        <div className="flex flex-col gap-2">
          <h3 className="mb-2 uppercase text-[#00000080] text-xs sm:text-sm">Education</h3>
      ...
      <div className="mt-32 sm:mt-48 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
        <h1
          ref={titleRef}
          className="text-[14vw] sm:text-[10vw] leading-[0.8] opacity-0 text-black transform translate-y-[50px]" // Added initial transform
        >
          Atlas AI
        </h1>
        <p
          ref={copyrightRef}
          className="opacity-0 text-sm sm:text-base"
        >
          © 2025 Atlas AI
        </p>
      </div>
